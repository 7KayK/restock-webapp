...
export function StoreMap() {
  ...
  // Best-effort background save of a GPS-derived fix as the user's location
  // in Settings, so next visit (and the Deals page, once it uses location)
  // can skip the GPS prompt entirely. Never blocks or fails the map itself.
  ...
  const requestLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        fetchStores(lat, lng)
        if (!hasSavedLocation.current) persistLocation(lat, lng)
      },
      () => {
        setStatus('denied')
      },
  ...
  }, [fetchStores, persistLocation])

  // On mount, prefer a location the user already saved in Settings — only
  // fall back to a live GPS prompt when there isn't one yet.
  useEffect(() => {
    if (!isLoaded) return
    let cancelled = false
    fetch('/api/settings')
      ...
      .then(
        (json: {
          data?: { locationLabel?: string | null; locationLat?: number | null; locationLng?: number | null }
        }) => {
          if (cancelled) return
          const { locationLabel: label, locationLat: lat, locationLng: lng } = json.data ?? {}
  ...
  }, [isLoaded, fetchStores, requestLocation])
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="text-center space-y-3 max-w-xs">
          ...
          <p className="text-xs text-[#132B22]/45">
            Enable location in your browser settings and try again, or set a location manually.
          </p>
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col gap-3">
      {locationLabel && (
        <p className="text-xs text-[#132B22]/50 px-0.5">
          Showing stores near <span className="font-medium text-[#132B22]/70">{locationLabel}</span>
          {' · '}
          <Link href="/dashboard/settings" className="underline hover:text-[#132B22]">
            Change
          </Link>
        </p>
      )}
