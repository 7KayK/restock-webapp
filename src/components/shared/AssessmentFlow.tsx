...
/**
 * The actual intro -> quiz -> results experience, with no opinion on how
 * it's presented. Two callers use this today:
 *  - AssessmentModal wraps it in a @base-ui/react Dialog for the site-wide
 *    popup + nav-triggered modal.
 *  - /assessment (via AssessmentPageClient) renders it directly on its own
 *    page, so Kay has a stable URL she can send to potential end users
 *    directly, independent of the site-wide popup feature flag.
 */
...
// Computed once, synchronously, as each state's lazy initializer — this is
// only ever called from a fresh client-side mount (AssessmentModal only
// mounts this after the user has already interacted, well past hydration;
// AssessmentPageClient gates its own mount until after hydration too), so
// there's no server/client hydration mismatch to worry about here.
...
export function AssessmentFlow({
  ...
  /** Omit to hide the close button entirely (e.g. on the standalone page). */
  onClose?: () => void
  /** Lets a modal wrapper know the current stage/step for its own close-time analytics. */
  onStateChange?: (state: { stage: Stage; step: number }) => void
}) {
  const [init] = React.useState(computeInitialState)
  const [stage, setStage] = React.useState<Stage>(init.stage)
  const [sessionId] = React.useState<string>(init.sessionId)
  const [step, setStep] = React.useState(init.step)
  const [contact, setContact] = React.useState<ContactInfo>(init.contact)
  const [bp, setBp] = React.useState<BestPracticeAnswers>(init.bp)
  const [qual, setQual] = React.useState<QualifyingAnswers>(init.qual)
  const [result, setResult] = React.useState<AssessmentResult | null>(null)
  const [submitting, setSubmitting] = React.useState(false)
  const [submitError, setSubmitError] = React.useState<string | null>(null)

  React.useEffect(() => {
    onStateChange?.({ stage, step })
  }, [stage, step, onStateChange])
  ...
  return (
    <div className="flex flex-col">
      {/* Header: progress (quiz only) + close (only when a close handler is given) */}
